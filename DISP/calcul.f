...
C--------------------------------------------------------------------
     ...
     *,cL(200,2)
	common/dispe1/rR(200,2),rL(200,2)
        common/amplification/ampl_corr(200)
...
C****************************************************
        ampl_corr(lip)=1./sqrt(ugr*sumi0)*1.E-06
        ugr0(lip)=ugr
        ccc0(lip)=ccc
...
c        write (27,*)amprfi(1), depth(1),' F(Hz)=',1000./t(lip), 'mode=',
c     *iq      
        do  m=2,nmax 
          if(amprfi(m).eq.0.0)go to 3333
c          write (27,*)amprfi(m),  depth(m)
        enddo         

c3333    write (27,'(/)'  )
3333    write(*,*) "strange here"
        if(ncount.eq.1.and.KEY_EIGEN_DER1) then
c	  write (31,*)'LOVE_H',' F(Hz)=',1000./t(lip)
          do  m=1,ndpth
C-------LOVE WAVE HORIZONTAL EIGENFUNCTION AND ITS DEPTH DERIVATIVES;
c            write(31,*  )dpth(m),dsprfi(m),(drvrfi(lm,m),lm=1,nderiv)
          enddo
        endif
...
C ****************************************************************          
c          do i=1,mmax
c            write(*,*) 'check bf amp',b(i),a(i),rho(i),d(i)
c          enddo

          ampl_corr(lip)=1./sqrt(ugr*sumi0)*1.E-06
c          write(*,*) ampl_corr(lip),lip,ugr,sumi0
          ugr0(lip)=ugr
          ccc0(lip)=ccc
c ****************************************************************
	  if(ncount.eq.1)then
...
3334    continue        
        ...
        if(ncount.eq.1.and.KEY_EIGEN_DER1) then
c	  write (32,*)'RAYL_V',' F(Hz)=',1000./t(lip)
c	  write (31,*)'RAYL_H',' F(Hz)=',1000./t(lip)
          do  m=1,ndpth
...
c	  write(nfile,*)1000./t(i),du1
c	  write(nfile+1,*)1000./t(i),du2
	enddo
